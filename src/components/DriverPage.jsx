...
import {
  ...
  Navigation, MapPin, Package, Check, CheckCircle2, ChevronLeft,
} from 'lucide-react';
import { fmt } from '../data/catalog.js';
import {
  getDriverSession, setDriverSession, clearDriverSession,
  driverLogin, driverListOrders, driverUpdateDelivery,
} from '../lib/driver.js';
    ...
    <div className="min-h-screen bg-night-900 font-body text-cream" dir="rtl">
      ...
      <main className="mx-auto max-w-2xl space-y-4 px-4 py-5">
        ...
        {loading ? (
          ...
          <div className="space-y-3">{shown.map((o) => <DeliveryCard key={o.id} o={o} onAdvance={advance} />)}</div>
        )}
...
/* ───────────── Delivery card ───────────── */
function DeliveryCard({ o, onAdvance }) {
      ...
      <div className="mt-3 flex items-center gap-1">
        {STEPS.map(([k, label], i) => (
          <div key={k} className="flex flex-1 flex-col items-center gap-1">
            <div className={`h-2 w-full rounded-full ${i <= curIdx ? 'bg-copper' : 'bg-white/10'}`} />
            <span className={`text-[9px] ${i <= curIdx ? 'text-copper-light' : 'text-cream/30'}`}>{label}</span>
          </div>
        ))}
      </div>
