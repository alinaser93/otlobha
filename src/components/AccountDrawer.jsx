          ...
          >
            ...
            <div className="flex items-center justify-between bg-brand-900 px-5 py-4 dark:bg-night-700">
              <div className="flex items-center gap-3">
                {account.avatar_url ? (
                  <img src={account.avatar_url} alt="" className="h-11 w-11 rounded-full object-cover ring-2 ring-copper/50" />
                ) : (
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              ...
              {/* profile details — professional editor */}
              <div className="rounded-3xl bg-cream p-5 shadow-soft dark:bg-night-800">
                <div className="mb-3 font-display text-lg font-extrabold text-ink dark:text-cream">بياناتي</div>
                <ProfileForm
                  initial={account}
                  readOnly={[{ label: 'الهاتف', value: account.phone, icon: Phone }]}
                  areas={AREAS}
                  show={{ email: true, phone2: true, birthdate: true, gender: true, area: true, address: true }}
                  uploadPrefix="customer"
                  uploadId={account.id}
                  onAvatarChange={async (url) => updateProfile({ avatar_url: url })}
                  onSave={async (vals) => updateProfile({
                    name: vals.name.trim(), area: vals.area, address: vals.address.trim(),
                    email: vals.email.trim() || null, phone2: vals.phone2.trim() || null,
                    birthdate: vals.birthdate || null, gender: vals.gender || null,
                  })}
                />
              </div>
            </div>
