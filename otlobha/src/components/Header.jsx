...
import { Leaf, ShoppingCart, Menu, X, MapPin } from 'lucide-react';

const LINKS = ['الباقات', 'الخضار', 'الفواكه', 'المؤونة', 'عروض'];

export default function Header({ cartCount, bump, onCart, cartRef }) {
  ...
  return (
    <motion.header
      ...
      className={`sticky top-0 z-50 transition-all duration-500 ${
        solid ? 'bg-brand-900/95 shadow-soft backdrop-blur-md' : 'bg-transparent'
      }`}
    >
            ...
            <AnimatePresence>
              {cartCount > 0 && (
                <motion.span
                  ...
                  className="absolute -left-1.5 -top-1.5 grid h-6 min-w-6 place-items-center rounded-full bg-cream px-1 text-[12px] font-extrabold text-brand-900 ring-2 ring-brand-900"
                >
                  {cartCount}
                </motion.span>
              )}
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            ...
            className="overflow-hidden bg-brand-900/98 backdrop-blur-md lg:hidden"
          >
